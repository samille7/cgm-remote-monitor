...
client.load = function load (serverSettings, callback) {
  ...
  $(document).keypress(function(e) {
    if (alarmingNow())
    {
      var bgParsedVal =   parseInt($('.bgStatus .currentBG').text());
      if (isNaN(bgParsedVal))
      {
        console.log("not a number");
      }
      else if (bgParsedVal < 61 && e.which != 43)
      {
        stopAlarm(true, 60000); // 1 min
      }
      else
      {
        stopAlarm(true, 3600000); // 60 min
      }
    }
  });
